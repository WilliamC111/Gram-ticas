document.addEventListener('DOMContentLoaded', function () {
    ...
    generateGrammarBtn.addEventListener('click', function () {
        ...
        for (let row of rows) {
            ...
            if (variable) {  // Variable es obligatoria
                ...
                for (const alt of alternatives) {
                    if (alt === '') {  // Producción vacía = epsilon
    ...
    generateDerivationsBtn.addEventListener('click', function () {
        ...
        .then((data) => {
            if (typeof data === 'object') {
                derivationTreeDiv.innerHTML = `<h5 class="mt-3">Árbol de Derivación:</h5><pre>${JSON.stringify(data, null, 2)}</pre>`;
            } else {
